...
namespace Cudafy
{
    ...
    public abstract class KernelMemberInfo
    {
        ...
        /// Checks if the assembly checksum and deserialized checksum are the same.
        /// </summary>
        /// <exception cref="CudafyException">Checksums do not match.</exception>
        public void VerifyChecksums()
        {
            if (!TryVerifyChecksums())
                throw new CudafyException(CudafyException.csCHECKSUM_FOR_ASSEMBLY_X_DOES_NOT_MATCH_DESERIALIZED_VALUE, Type.Assembly.FullName);
        }
