...
namespace Cudafy.Maths.BLAS
{
    ...
    public abstract class GPGPUBLAS : IDisposable
    {
        ...
        public void ConvertBandedMatrixCBC<T>(T[] bandedMatrix, T[] cbcMatrix, int m, int n, int kl, int ku)
        {
            for (int i = 0; i < m; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    if ((i >= j - ku) && (i <= j + kl))
                    {
                        ...
                        if (index >= 0 && index < bandedMatrix.Length)
                        {
                            cbcMatrix[packedIndex] = bandedMatrix[index];
                        }
                    }
                }
            }
        ...
        public void ConvertBandedSymmetricMatrixCBC<T>(T[] bandedMatrix, T[] cbcMatrix, int n, int k, cublasFillMode fillMode)
        {
            if (fillMode == cublasFillMode.Lower)
            {
                for (int i = 0; i < n; i++)
        ...
        public void PackSymmetricMatrix<T>(T[] symmetrixMatrix, T[] packedMatrix, int n, cublasFillMode fillMode)
        {
            if (fillMode == cublasFillMode.Lower)
            {
                for (int j = 0; j < n; j++)
                {
                    for (int i = j; i < n; i++)
                    {
                        packedMatrix[GetIndexPackedSymmetric(i, j, n, fillMode)] = symmetrixMatrix[GetIndexColumnMajor(i, j, n)];
                    }
                }
            }
            else
